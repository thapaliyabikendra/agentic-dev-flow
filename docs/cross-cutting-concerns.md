...
The version is captured in every Validation Log's audit reproducibility set as `baseline_version` (the snapshot variable name is preserved for backward compatibility with prior logs).
...
| 2.0 | 2026-04-28 | refactor | Renamed from `frs-platform-baseline.md` to `cross-cutting-concerns.md`; relocated from plugin's `shared/` to project's `docs/`; wrapper terminology updated ("Platform Baseline" → "Cross-Cutting Concerns"); category names and content unchanged; ownership shifted to `agent:cross-cutting-curator` |
| 2.1 | 2026-04-28 | refactor | Parameterized banking defaults (retention duration, operating timezone, compliance role) for project-agnostic reuse; tagged each category with `**Origin:** baseline (template v1.0)` to distinguish from future project-added categories; aligned with plugin-shipped cross-cutting-concerns template v1.0. |
